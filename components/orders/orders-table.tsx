...
function StatusDropdown({ order, statuses, statusMap }: { order: Order; statuses: OrderStatusConfig[]; statusMap: Record<string, { label: string; color: string; dot: string }> }) {
  ...
  const [dropPos, setDropPos] = useState({ top: 0, left: 0, openAbove: false })
  ...
  function handleOpen(e: React.MouseEvent) {
    ...
    if (!open && btnRef.current) {
      ...
      setDropPos({
        top: openAbove ? rect.top - 8 : rect.bottom + 6,
        left: rect.left,
        openAbove,
      })
    }
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={isPending}
        className={cn(
          'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium border transition-all hover:shadow-sm active:scale-95',
          current.color,
      ...
      {open && typeof window !== 'undefined' && createPortal(
        <>
          ...
          <div
            className={cn(
              'fixed z-[999] w-52 rounded-2xl border border-zinc-200 bg-white p-1.5 shadow-xl shadow-black/10 dark:border-zinc-700 dark:bg-zinc-900',
              'animate-scale-in'
            )}
            style={dropPos.openAbove
          ...
          >
            <div className="px-2.5 pb-1.5 pt-1">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Сменить статус</p>
            </div>
            {statuses.map((s) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[12px] font-medium transition-colors',
                    isActive
  ...
  return (
    <div className="relative">
      <button
        ref={btnRef}
        onClick={handleOpen}
        className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800/60 dark:hover:text-zinc-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/50"
      >
      ...
      {open && typeof window !== 'undefined' && createPortal(
        <>
          ...
          <div
            className={cn(
              'fixed z-[999] w-48 rounded-2xl border border-zinc-200 bg-white p-1.5 shadow-xl shadow-black/10 dark:border-zinc-700 dark:bg-zinc-900',
              'animate-scale-in'
            )}
            style={dropPos.openAbove
              ? { bottom: window.innerHeight - dropPos.top, left: dropPos.left }
          ...
          >
            <button
              type="button"
              className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[12px] font-medium text-zinc-600 hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800/60 transition-colors"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setOpen(false); router.push(`/orders/${order.id}`) }}
            >
            ...
            {(order.phone || order.client?.phone) && (
              ...
              <a
                ...
                className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[12px] font-medium text-green-600 hover:bg-green-50 dark:hover:bg-green-950/30 transition-colors"
                onClick={(e) => e.stopPropagation()}
              >
                <Phone size={13} />
                WhatsApp
            ...
            {userRole === 'admin' && (
              <>
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[12px] font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
                >
                  <Trash2 size={13} />
                  Удалить заказ
                </button>
